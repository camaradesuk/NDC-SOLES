          ...
          interventions_in_cat() %>%
            ...
            layout(showlegend = TRUE,
                   ...
                   xaxis = list(title = "", 
                                ...
                                type = "category",
                                categoryorder = "array", 
                                categoryarray = seq(min(input$pico_year_slider), 
                                                    max(input$pico_year_slider))), 
                   barmode = 'dodge',
                   hoverlabel = list(bgcolor = "white", 
                                     font = list(size = 14)),
                   annotations =
...
search_UI <- function(id, table) {
  ...
  tagList(
    ...
    box(width= 12,
        ...
        tags$head(tags$style("#search_results_text{color: black;
                                       font-size: 20px;
                                       font-style: italic;
                                       }"
        )),
        ...
        div(style="display: inline-block;vertical-align:top; width: 50px;",
            ...
            dropdown(inputId = ns("dropdown_menu"),
                     ...
                     sliderInput(ns("year_slider"), 
                                 ...
                                 as.numeric(max(table$year, na.rm=TRUE)),
                                 value = c(as.numeric(min(table$year, na.rm=TRUE)), as.numeric(max(table$year, na.rm=TRUE))), sep=""),
    ...
    function(input, output, session) {
      ...
      observeEvent(input$reset_search, {
        ...
        updateSliderInput(session = session,
                          ...
                          min = as.numeric(min(table$year, na.rm=TRUE)),
                          max = as.numeric(max(table$year, na.rm=TRUE)),
                          value = c(as.numeric(min(table$year, na.rm=TRUE)), as.numeric(max(table$year, na.rm=TRUE))))
      ...
      output$search_results_text <- renderText({
        ...
        # If there is an advanced search query
          ...
          if(values$submit_filters == "clicked" & values$search_query == "NOT BLANK"){
            ...
            paste0("Your translated search query tells the application to find STROKE-SOLES papers with a regex match to ", translated_query,
            ...
            paste0("Your translated search query tells the application to find STROKE-SOLES papers with a regex match to ",
                   translated_query,
